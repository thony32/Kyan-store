...
const Carousel: React.FC<CarouselProps> = ({ children, options, className, activeSlider, isScale = false, isAutoPlay = false }) => {
...
export const SliderPrevButton = ({
    ...
    return (
        <button className={cn('', className)} type="button" onClick={onPrevButtonClick} disabled={prevBtnDisabled}>
            {children}
        </button>
    )
}
export const SliderNextButton = ({
    ...
    return (
        <>
            <button className={cn('', className)} type="button" onClick={onNextButtonClick} disabled={nextBtnDisabled}>
                {children}
            </button>
        </>
    )
...
export const SliderProgress = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={cn('  bg-gray-500 relative rounded-md h-2 justify-end items-center w-96 max-w-[90%] overflow-hidden', className)}>
            <div
...
export const SliderSnapDisplay = ({ className }: { className?: string }) => {
    ...
    return (
        <div className={cn('mix-blend-difference overflow-hidden flex gap-1 items-center', className)}>
            <AnimatedSnap selectedSnap={selectedSnap} direction={direction} />
            <span>/ {snapCount}</span>
        </div>
    )
}

const createVariants = (direction: number) => ({
...
const AnimatedSnap = ({
    ...
    // Create the variants using the direction prop
    const variants = createVariants(direction)

    return (
        <AnimatePresence mode="wait" initial={false} custom={direction}>
            <motion.div key={selectedSnap} custom={direction} variants={variants} initial="initial" animate="animate" exit="exit">
                {selectedSnap + 1}
            </motion.div>
        </AnimatePresence>
    )
...
export const SliderDotButton = ({ className }: { className?: string }) => {
    const { selectedIndex, scrollSnaps, onDotButtonClick, carouselId }: any = useCarouselContext()
    return (
        <div className={cn('flex', className)}>
            <div className="flex gap-2">
                {scrollSnaps.map((_: any, index: React.Key | null | undefined) => (
                    ...
                    >
                        <div className=" bg-gray-500/40  h-2 rounded-sm w-10 " />
                        {index === selectedIndex && (
...
export const ThumsSlider: React.FC<any> = () => {
    const { emblaThumbsRef, slidesrArr, selectedIndex, onThumbClick }: any = useCarouselContext()
            ...
            <div className="flex flex-row gap-2">
                {slidesrArr?.map((slide: any, index: any) => (
                    ...
                    <div
                        // biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
                        key={index}
                        className={`min-w-0 w-full xl:size-24 cursor-pointer aspect-auto border-2  rounded-md ${
                            index === selectedIndex ? '  opacity-100' : 'border-transparent opacity-30 '
                        }`}
                        style={{ flex: '0 0 15%' }}
                        onClick={() => onThumbClick(index)}
                    >
                        <motion.img src={slide} className="w-full h-full object-cover rounded-sm" width={400} height={400} alt={`image-${index}`} />
                    </div>
                ))}
            </div>
        </div>
